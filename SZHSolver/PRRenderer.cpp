...
env_map_path(env_map_path)
{
    ...
    m_shader_bubble.loadFromFile(env_map_path+"PR");
...
void PRRenderer::render()
{
    ...
    for (size_t i = 0; i < mesh().nt(); i++)
    {
        ...
        // print(t[])
        Vec3 x0(hgf->pos(t[0]).x(), hgf->pos(t[0]).y(), hgf->pos(t[0]).z());
        Vec3 x1(hgf->pos(t[1]).x(), hgf->pos(t[1]).y(), hgf->pos(t[1]).z());
        Vec3 x2(hgf->pos(t[2]).x(), hgf->pos(t[2]).y(), hgf->pos(t[2]).z());
        Vec3 c = (x0 + x1 + x2) / 3;
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            for (size_t k = 0; k < mesh().m_vertex_to_triangle_map[t[j]].size(); k++)
            {
                ...
                if (rp == rpn)
                {
                    ...
                    Vec3 xn0(hgf->pos(tn[0]).x(), hgf->pos(tn[0]).y(), hgf->pos(tn[0]).z());
                    Vec3 xn1(hgf->pos(tn[1]).x(), hgf->pos(tn[1]).y(), hgf->pos(tn[1]).z());
                    Vec3 xn2(hgf->pos(tn[2]).x(), hgf->pos(tn[2]).y(), hgf->pos(tn[2]).z());
